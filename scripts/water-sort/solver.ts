...
export type SolverOutcome =
  ...
  | {
      status: "aborted";
      reason: "explored-state-budget" | "time-budget";
      exploredStateCount: number;
      maximumBranchingFactor: number;
      elapsedMilliseconds: number;
    }
...
export function solveWithAStarBounded(
  initialBoard: Board,
  capacity: number,
  limits: SolverLimits,
): SolverOutcome {
  if (limits.maxExploredStates <= 0)
    throw new Error("maxExploredStates must be positive.");
  if (limits.maxElapsedMilliseconds <= 0)
    throw new Error("maxElapsedMilliseconds must be positive.");
  ...
  while (queue.size > 0) {
    if ((exploredStateCount & 0x1ff) === 0) {
      ...
      if (elapsedMilliseconds >= limits.maxElapsedMilliseconds)
        return {
          status: "aborted",
          reason: "time-budget",
          exploredStateCount,
    ...
    if (node.costSoFar > (bestCost.get(nodeKey) ?? Number.POSITIVE_INFINITY))
      continue;
    exploredStateCount += 1;
    if (exploredStateCount > limits.maxExploredStates)
      return {
        status: "aborted",
        reason: "explored-state-budget",
        exploredStateCount,
        maximumBranchingFactor,
        elapsedMilliseconds: performance.now() - startedAt,
      };
    ...
    maximumBranchingFactor = Math.max(
      maximumBranchingFactor,
      legalMoves.length,
    );
...
export function solveWithAStar(
  ...
  const outcome = solveWithAStarBounded(initialBoard, capacity, {
    maxExploredStates,
...
export function verifySolution(
  ...
  try {
    for (const move of solution)
      board = applyMove(board, move, capacity).nextBoard;
  } catch {
    return false;
  }
