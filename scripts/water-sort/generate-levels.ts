...
interface PendingLevel { id: string; json: {id: string; capacity: number; vials: ColorId[][]; development: LevelDevelopmentMetadata}; }
...
async function seedSeenBoards(output: string, manifest: readonly ManifestEntry[], seen: Set<string>): Promise<void> { for (const entry of manifest) { const stored = JSON.parse(await readFile(path.join(output, `${entry.id}.json`), "utf8")) as StoredLevel; seen.add(createCanonicalBoardKey(stored.vials.map((vial) => [...vial].reverse()))); } }
...
async function main(): Promise<void> {
  ...
  const output = path.resolve(flag("--output") ?? "levels");
  ...
  for (let attempt = 1; attempt <= maximumAttempts && pendingLevels.length < requestedCount; attempt += 1) {
    ...
    pendingLevels.push({id, json: {id, capacity, vials: board.map((vial) => [...vial].reverse()), development}});
  ...
  for (const level of pendingLevels) { await writeFile(path.join(output, `${level.id}.json`), `${JSON.stringify(level.json, null, 2)}\n`, "utf8"); manifest.push({id: level.id}); }
