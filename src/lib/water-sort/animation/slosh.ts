import {getPourVisualPhase} from "./pour-visuals";
...
export interface SourceMotionSnapshot {x: number; y: number; rotationDegrees: number}
...
export function createLiquidSimulation({sourceWidthPixels, getSourceMotion, destinationImpactXNormalized}: {
  sourceWidthPixels: number;
  getSourceMotion: () => SourceMotionSnapshot;
  destinationImpactXNormalized: number;
}): LiquidSimulation {
  const metresPerPixel = 0.06 / Math.max(1, sourceWidthPixels);
  const homeTimeSeconds = GAME_TIMING.pour.returnTravelSeconds + GAME_TIMING.pour.returnTravelDurationSeconds;
  const destinationImpactSample = clamp(destinationImpactXNormalized, 0, 1) * (SURFACE_POINT_COUNT - 1);
  ...
  function integrate(deltaSeconds: number, equilibriumWorldAngle: number, integrationTimeSeconds: number): void {
    ...
    for (let step = 0; step < substepCount; step += 1) {
      const sourceAcceleration = SOURCE_NATURAL_FREQUENCY * SOURCE_NATURAL_FREQUENCY
      ...
      sourceWorldAngle = clamp(sourceWorldAngle + sourceAngularVelocity * substepSeconds,
        -MAX_SOURCE_WORLD_ANGLE_DEGREES, MAX_SOURCE_WORLD_ANGLE_DEGREES);
      const previousDisplacements = [...destinationDisplacements];
      const previousVelocities = [...destinationVelocities];
      const flow = getPourVisualPhase(integrationTimeSeconds).streamStrength;
      for (let pointIndex = 0; pointIndex < SURFACE_POINT_COUNT; pointIndex += 1) {
        const left = pointIndex > 0 ? previousDisplacements[pointIndex - 1] ?? 0 : previousDisplacements[pointIndex] ?? 0;
        const right = pointIndex < SURFACE_POINT_COUNT - 1 ? previousDisplacements[pointIndex + 1] ?? 0 : previousDisplacements[pointIndex] ?? 0;
        ...
        const distanceFromImpact = pointIndex - destinationImpactSample;
        const impactWeight = Math.exp(-(distanceFromImpact * distanceFromImpact) / 3.2);
        const acceleration = -DESTINATION_SPRING * displacement - destinationDamping * velocity
          + neighborForce + DESTINATION_IMPACT_FORCE * flow * impactWeight;
        ...
        destinationDisplacements[pointIndex] = clamp(
          ...
          -MAX_DESTINATION_DISPLACEMENT, MAX_DESTINATION_DISPLACEMENT,
        );
      }
    }
  }
  ...
  function update(timeSeconds: number): void {
    ...
    const equilibriumWorldAngle = clamp((Math.atan2(horizontalAcceleration, denominator) * 180) / Math.PI, -18, 18);
    ...
    if (!impactTriggered && timeSeconds >= impactTime) {
      ...
      for (let pointIndex = 0; pointIndex < SURFACE_POINT_COUNT; pointIndex += 1) {
        const distance = pointIndex - destinationImpactSample;
        const weight = Math.exp(-(distance * distance) / 2.4);
        destinationVelocities[pointIndex] = (destinationVelocities[pointIndex] ?? 0) + 30 * weight;
      }
    }
  ...
  function reset(): void {
    previousTimeSeconds = null; previousCenterX = 0; previousCenterY = 0;
    previousVelocityX = 0; previousVelocityY = 0; filteredAccelerationX = 0; filteredAccelerationY = 0;
    sourceWorldAngle = 0; sourceAngularVelocity = 0; impactTriggered = false; currentTimeSeconds = 0;
    currentRotationDegrees = 0; destinationDisplacements.fill(0); destinationVelocities.fill(0);
  }
  function finish(): void {
    currentTimeSeconds = GAME_TIMING.pour.totalSeconds; sourceWorldAngle = 0; sourceAngularVelocity = 0;
    filteredAccelerationX = 0; filteredAccelerationY = 0; currentRotationDegrees = 0;
    destinationDisplacements.fill(0); destinationVelocities.fill(0);
  }
  function getSnapshot(): LiquidSimulationSnapshot {
    ...
    return {
      ...
      destinationMaximumDisplacement: Math.max(0, ...destinationDisplacements.map((value) => Math.abs(value))),
