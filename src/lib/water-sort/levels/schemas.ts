...
export const DevelopmentMetadataSchema = z.object({
  ...
  generationDepth: z.number().int().nonnegative().optional(),
  generator: z.literal("uniform-shuffle").optional(),
  generationSeed: z.number().int().nonnegative().optional(),
  solverElapsedMilliseconds: z.number().nonnegative().optional(),
  difficultyScore: z.number().min(0).max(1).optional(),
  difficultyScoreVersion: z.number().int().positive().optional(),
});

export const RawLevelSchema = z.object({
  id: z.string().min(1), capacity: z.number().int().min(2).max(8), vials: z.array(z.array(ColorIdSchema)), development: DevelopmentMetadataSchema.optional(),
}).superRefine((level, context) => {
  const counts = new Map<string, number>();
  level.vials.forEach((vial, vialIndex) => {
    if (vial.length > level.capacity) context.addIssue({code: "custom", path: ["vials", vialIndex], message: `Vial exceeds capacity ${level.capacity}.`});
...
export function rawLevelToLevel(rawLevel: RawLevel): Level {
  ...
  return {id: rawLevel.id, capacity: rawLevel.capacity, board, ...(rawLevel.development === undefined ? {} : {development: rawLevel.development})};
}
export function boardToJsonVials(board: Board): RawLevel["vials"] { return board.map((vial) => [...vial].reverse()); }
